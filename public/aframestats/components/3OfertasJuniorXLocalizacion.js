// OfertasJuniorXLocalizacion.js
document.addEventListener("DOMContentLoaded", function () {
  const buttons = document.querySelectorAll(".toggle-button3");
  ...
  async function toggleGraph(button3) {
    const graph = document.getElementById("graph3");

    if (graph) {
      graph.parentNode.removeChild(graph);
      button3.setAttribute("src", "./textures/on.png");
      button3.setAttribute("data-status", "on");
    } else {
      await fetchAndRenderGraph();
      button3.setAttribute("src", "./textures/off.png");
      button3.setAttribute("data-status", "off");
    }
  }
      ...
      if (response.status === 200) {
        ...
        data.forEach((job) => {
          if (job.junior && job.location) {
            ...
            juniorCount[locationKey] = (juniorCount[locationKey] || 0) + 1;
          }
        });

        const remotoCount = Object.entries(juniorCount)
          .filter(([location]) => location === "remoto")
          .reduce((total, [, count]) => total + count, 0);
        const otherLocations = Object.entries(juniorCount)
          .filter(([location]) => location !== "remoto")
          .reduce(
  ...
  function renderBarGraph(data) {
    ...
    graphEntity.setAttribute("id", "graph3");
    ...
    const existingGraph = document.getElementById("graph3");
    if (existingGraph) {
